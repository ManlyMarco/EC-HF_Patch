...
MsgIncompatibleModsDetected=WARNING - Incompatible mods have been detected in your game folder! You are using very outdated mods or mods from other games in EmotionCreators, which WILL cause problems. All old mods will have to be removed to fix this.
MsgExeNotFound=WARNING - EmotionCreators.exe was not found in selected directory. This patch has to be installed directly to the main EmotionCreators game directory in order to work properly.%n%nAre you sure that this directory is correct?
...
MsgKoikatuDetected=ERROR - Koikatsu files have been detected! If you installed EmotionCreators to the same folder as Koikatsu, you have broken Koikatsu game files. To fix this you will need to reinstall both games to separate directories!
...
IconGame=EmotionCreators launcher
...
jp.MsgIncompatibleModsDetected=警告 - あなたのゲームフォルダで互換性のない改造が検出されました！ あなたは非常に古くなったモッズを使っているか、「エモーション・クリエイター」の他のゲームのモッズを使っています。 これを修正するには、古いモッズをすべて削除する必要があります。
jp.MsgExeNotFound=警告 - EmotionCreators.exeが選択されたディレクトリに見つかりませんでした。 正しく動作させるためには、このパッチを「エモーション・クリエイター」のメインゲームディレクトリに直接インストールする必要があります。%n%nこのディレクトリが正しいことを確認していますか？
...
jp.MsgKoikatuDetected=エラー - 「コイカツ」を「エモーション・クリエイター」と同じフォルダにインストールした場合は、「コイカツ」ゲームファイルが壊れています。修正するには、両方のゲームを別々のディレクトリに再インストールする必要があります。
...
jp.RunGame=ゲームを始める
jp.RunWiki=オープン英語「エモーション・クリエイター」のwiki（FAQ、マニュアル）
...
jp.IconGame=エモーション・クリエイター ランチャー
